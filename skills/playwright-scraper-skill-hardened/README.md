...
A Playwright-based web scraping OpenClaw Skill with anti-bot protection and hardened security defaults.
...
- ✅ **Secure Defaults** — Blocks private/local network targets by default
...
State file (written by `ensure-runtime.sh`):
...
# Allow private/local network access (disabled by default)
ALLOW_PRIVATE_NETWORK=true node scripts/playwright-simple.js http://10.0.0.8

# Disable browser sandbox only when your environment requires it
...
- By default, the scripts only allow `http/https` URLs.
- By default, private/local/metadata network targets are blocked (SSRF protection).
- Sandbox is enabled by default; only set `DISABLE_SANDBOX=true` if absolutely necessary.
...
### Agent keeps reinstalling dependencies?

Use the idempotent runtime bootstrap:
...
`checkedAt` will update every run, while install only happens when runtime is missing/incomplete.
