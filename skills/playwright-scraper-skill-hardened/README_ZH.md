...
基於 Playwright 的網頁爬蟲 OpenClaw Skill。支援反爬保護，並加入安全預設（預設阻擋私網/本機目標）。
...
- 預設僅允許 `http/https` 目標 URL
- 預設阻擋私網/本機/雲 metadata 位址（降低 SSRF 風險）
- 預設啟用瀏覽器 sandbox；除非必要，不要設定 `DISABLE_SANDBOX=true`
...
`checkedAt` 每次會更新；只有在運行環境缺失/不完整時才會真的安裝。
